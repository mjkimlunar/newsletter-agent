...
세 곳뿐이었고, 검수·발행 로직은 **도메인 전환 때문에** 코드를 고칠 일은 없었다.
(다만 이후 검수·발행 쪽에서 도메인과 무관한 별개의 결함 두 가지 — 검수 프롬프트가
할루시네이션을 못 거르던 문제, 발행 제목이 "AI 브리핑"으로 하드코딩돼 있던 문제 —
를 실제 운영 중에 발견해 고쳤다. 5·6번 항목 참고.)
...
stateDiagram-v2
    ...
    note right of publish
        ⑤ 발행: 기본 Discord 웹훅
        PUBLISH_CHANNEL=email이면 이메일(SMTP)로 교체 (6번 참고)
...
누적 기록된다. (이 시점 로그는 `⑤ 발행`으로만 찍히는데, 6번 항목에서 이메일
채널을 추가하며 `⑤ 발행(Discord)`/`⑤ 발행(이메일)`처럼 채널명을 덧붙이도록
바꿨다 — 아래 실행 예시들의 표기가 조금씩 다른 건 이 순서 때문이다.)
...
4) 고친 프롬프트로 **오늘자 실제 수집 데이터를 다시 돌리자, 인위적 조작 없이도
   ...
   **다만 이 사례를 다시 보면서 깨달은 것**: 이 뉴스레터의 목적은 "지금 무엇을
   패치해야 하는가"이고, 그 관점에서는 6개국이든 7개국이든, "스캔"이든
   "공격"이든 독자가 취할 행동("Gitea 최신 버전으로 업데이트")은 바뀌지 않는다
   — 그래서 실무적 전달력 자체는 크게 훼손되지 않았다. 하지만 만약 이게
   "이번 주 몇 건의 침해가 발생했는가"처럼 **통계·수치 자체가 콘텐츠의 핵심인**
   뉴스레터였다면 이 정도 숫자 부풀림은 훨씬 치명적인 오류였을 것이다. 예를
   들어 투자 유치 금액, 피해 규모(유출된 개인정보 건수, 손해액)처럼 **그 수치
   자체가 기사의 존재 이유인 경우**는 조치 권고와 무관하게 정확도가 곧 기사의
   신뢰도이므로 원문과의 대조를 지금보다 훨씬 엄격하게 가져가야 한다. 즉
   "검수가 무엇을 얼마나 엄격하게 봐야 하는가"도 소스·선별 기준과 마찬가지로
   독자 목적에 따라 달라져야 하는 값이지, 모든 뉴스레터에 동일한 엄격도를
   적용하면 되는 문제가 아니라는 걸 이번 케이스로 확인했다. 지금 `check()`는
   모든 주제에 대해 같은 엄격도로 판정하는데, `audience*.yaml`에 "숫자·통계가
   핵심 콘텐츠인가"를 표시하는 값을 추가해 검수 프롬프트의 엄격도를 그에 맞게
   조절하는 것도 다음 개선 후보로 남겨둔다.

   **이미 마련된 최소한의 안전망**: `_build_articles()`가 넘기는 `url`이 Discord
   임베드에서는 `url` 필드로, 이메일 HTML에서는 `<a href='{url}'>` 앵커로 각
   헤드라인에 걸려 있다. 즉 검수가 놓친 숫자가 있어도 독자가 헤드라인을 눌러
   원문에서 직접 대조할 수 있다 — LLM 기반 검수가 완전하지 않다는 걸 이번에
   확인한 만큼, 이 출처 링크가 결코 장식이 아니라 검수 실패에 대한 최후의
   방어선이라는 의미가 새삼 분명해졌다.
...
5) **정직하게 덧붙이면**: 이메일 채널 검증용으로 같은 파이프라인을 한 번 더
   돌렸을 때(6번 항목의 `result_email.png`), 바로 이 "Red Heron" 기사가
   또 한 번 초안에서 같은 종류의 과장(스캔→공격, 국가 수 부풀림)으로 작성됐는데
   이번엔 검수를 그냥 통과했다. 즉 프롬프트를 고쳐도 **매번 100% 잡아내는 건
   아니다** — 잡아낼 확률이 올라갔을 뿐, 여전히 확률적이라는 걸 역설적으로
   다시 확인한 셈이다. 근본적으로 없애려면 검수 자체도 온도를 낮추거나
   여러 번 판정해 다수결을 취하는 등의 보강이 더 필요하다는 게 다음 개선
   과제로 남는다.
...
독립적으로 판단하기 때문), ⑤만 노드가 교체된다.

**실제 렌더링된 이메일 화면**: `build_email_html()`이 만든 HTML을 파일로 저장해
브라우저로 열어본 화면이다 (SMTP 발송 없이 결과물만 직접 확인).

![이메일 발행 미리보기](result_email.png)

Discord 임베드와 같은 헤드라인·요약·💡 인사이트·출처 구조를 그대로 HTML 문서로
옮긴 것을 확인할 수 있다. `SMTP_HOST`/`SMTP_USER`/`SMTP_PASS`/`MAIL_TO`를 설정하면
실제 발송도 가능하지만, 이번 제출에서는 구조와 dry-run 동작·렌더링 결과만
...
**폭주 방지 장치**: 그래프(`build()`)는 `collect → select → (report*) → verify → publish`
순서의 비순환(acyclic) 구조라 조건 분기가 있어도 되돌아가는 경로가 없다 — 구조상
무한 루프가 불가능하다. 그 안에서도 개수는 전부 상한이 있다: 선별은 예선
`BATCH=40`·본선 `TARGET=5`로 고정, `report`의 `Send` fan-out도 `picked`(최대 5개)
만큼만 열린다. RSS 수집(`requests.get(..., timeout=20)`)과 웹훅 전송에는 20초
타임아웃이 걸려 있다. **정정(동료 리뷰 2026-09-16)**: 본문 추출(`extract_body()`의
`trafilatura.fetch_url()`)은 `requests`를 거치지 않고 자체 내부 요청을 쓰기 때문에
이 `timeout=20`이 적용되지 않는다 — 이전 버전의 이 문서는 "본문 추출도 20초
타임아웃이 걸려 있다"고 잘못 적어 뒀었다. 운영 레벨에서는 `.github/workflows/
security.yml`에 `timeout-minutes: 20` 잡 타임아웃이 걸려 있어, 본문 추출이 오래
걸리는 경우를 포함해 위 장치가 다 뚫려도 GitHub Actions가 강제 종료한다.

**엣지 케이스 테스트**:

1) *수집 0건* — `hours=0`으로 강제해 사실상 아무 기사도 안 걸리는 상황을 만들었다.
   ```
   ① 수집   0시간 창 · 0건
   ② 선별   0 → 0건
   ④ 검수   0 → 0건
   ⑤ 발행(Discord)   조용합니다 · dry-run
   ```
   예외 없이 끝까지 진행되고, `build_embeds()`가 "오늘은 조용합니다" embed 1개로
   정상 발행한다 — 아무 것도 없는 날을 별도 분기 없이 "0건짜리 정상 케이스"로
   처리하도록 설계돼 있어서 특별한 예외 처리 코드가 필요 없었다.
2) *응답 불가능한 소스 혼입* — 존재하지 않는 도메인을 `SOURCES`에 강제로
   끼워넣었다.
   ```
   ① 수집   24시간 창 · 31건 · 응답 없음 ['가짜소스']
   ② 선별   31 → 예선 8 → 5건
   ④ 검수   5 → 4건 · 불합격 ['The Hacker News']
   ⑤ 발행(Discord)   4건 · dry-run
   ```
   `collect()`의 `try/except`가 죽은 소스를 로그에만 남기고 나머지 소스로
   계속 진행한다 — 여기서도 검수가 또 다른 기사(이번엔 The Hacker News)를
   자연스럽게 걸러내, 검수 로직이 특정 소스에 국한된 우연이 아니라는 근거가
   하나 더 쌓였다.

**소스코드 출처**: 이 프로젝트는 처음부터 새로 짠 것이 아니라 수강 중인 뉴스레터
에이전트 커리큘럼의 실습 결과물을 기반으로 한다. 어디까지가 커리큘럼이고 어디부터
이번 제출을 위해 직접 붙인 것인지 구분하면:

- **커리큘럼에서 그대로 가져온 것** — `collect`/`select`/`report`/`verify`/`publish`
  5노드 구조와 `Brief` State 설계(노트 02·05·07·08·09·10, "걸어다니는 뼈대" 방식),
  예선→본선 2단계 선별과 `BATCH`/`TARGET` 개념(노트 07), `AUDIENCE_CONFIG`
  환경변수로 설정 파일을 갈아 끼우는 패턴 자체(노트 13 "내 분야로 옮기기"),
  GitHub Actions `schedule`+`workflow_dispatch` 이중 트리거와 `DRY_RUN`
  안전장치 개념(노트 11).
- **커리큘럼이 "더 해보기"로 제안했고 이번에 실제로 구현한 것** — `select()`의
  `event` 라벨 기반 코드 중복 제거(노트 07 "더 해보기" 3번이 제안한 방향인데,
  실제로 프로덕션에서 중복 발행을 발견한 뒤에야 구현했다).
- **커리큘럼에 없고 이번 제출을 위해 직접 추가·발견한 것** — 보안 주제 전체
  (`audience_security.yaml` 및 소스 8곳 실측·채택표), 실측 중 발견한 Dark
  Reading 403 문제와 SecurityWeek 교체, 검수(`SYS_CHECK`) 프롬프트가 실제로
  아무것도 못 거르던 문제와 그 수정, 발행 제목 "AI 브리핑" 하드코딩 버그와
  `브리핑명` 설정 분리, 이메일 발행 채널(`publish_email`/`build_email_html`/
  `send_email`), 이번 8번 항목의 폭주 방지 장치 설명과 엣지 케이스 테스트.

**AI와 협업하며 사람(코더)이 직접 판단·발견한 부분**: 이 프로젝트는 Claude Code와
페어로 작업했고, 코드 대부분은 AI가 작성했지만 아래는 사람이 직접 결정하거나
실제로 발견한 지점이다.
- 제출 주제를 "보안"으로 정한 것, 그리고 기존에 이미 운영 중이던 보안 뉴스레터
  파이프라인을 이번 제출에 재사용하기로 한 결정.
- **실제 발행된 뉴스레터를 읽다가 GitLab 취약점 기사 두 개가 같은 내용이라는 걸
  직접 알아채고 신고**했다 (AI가 로그만 봐서는 못 잡아낸 문제 — 3번 항목의
  `event` 중복 제거 버그 수정은 이 신고에서 시작됐다).
- 검수(④) 절 스크린샷을 검토하다가 "이 기사는 조치 중심이라 숫자가 좀 부풀어도
  전달력엔 문제가 없지만, 통계 자체가 콘텐츠인 뉴스레터였다면 훨씬 심각했을 것"
  이라는 관점 — 5번 항목의 "검수 엄격도도 독자 목적에 따라 달라져야 한다"는
  논의는 이 지적에서 나왔다.
- 발행 채널을 Discord 외에 이메일로도 확장해볼지, 실제 SMTP 발송까지 할지
  아니면 dry-run 구조 검증까지만 할지 같은 범위 결정.
- 이 REPORT.md를 낯선 리뷰어가 읽었을 때 앞뒤가 맞는지 스스로 점검하도록 요청 —
  그 결과 1번 항목의 자기모순 문장, 4번 다이어그램과 6번 이메일 채널 간 불일치를
  실제로 찾아 고쳤다.

**작업 흔적**: 진행 과정은 GitHub 커밋 이력에 순서대로 남아 있다 — 각 커밋 메시지가
"무엇을 왜 고쳤는지"(예: `1ace883` 소스 교체 이유, `2d7a602` 검수 프롬프트 보강
이유)를 설명하는 식으로 작성했다.

❓ **리뷰어가 물어볼 만한 질문에 대한 답 (예시)**: "검수 프롬프트를 고쳤는데도 왜
또 놓치는 사례가 나왔나?" → 5번 항목 5)번 참고 — 프롬프트 개선은 확률을 낮출 뿐
0으로 만들지는 못한다는 걸 실제로 확인했고, 완전히 없애려면 검수 자체를 다수결
방식으로 여러 번 판정하는 보강이 필요하다고 결론 내렸다(다음 개선 과제).

### 동료 리뷰 반영 (2026-09-16)

신서연님의 피어리뷰(코드 라인까지 짚은 상세 버전)에서 지적받은 것 중 8건을
실제로 코드로 고쳤다. 전부 로컬 dry-run으로 재검증했다.

1. **`draft()` 6,000자 / `check()` 5,000자 불일치** — 둘 다 `BODY_CHARS = 6000`
   상수 하나로 통일. 5,001~6,000자 구간의 사실이 "원문에 없음"으로 오판될 수
   있었던 경로를 없앴다.
2. **발송 실패와 dry-run이 로그에서 구분 안 됨** — `send()`/`send_email()`이
   이제 `bool` 대신 `"dry-run"/"sent"/"failed"` 문자열을 돌려주고, `Brief`
   State에 `sent_status` 칸을 추가해 `metrics.jsonl`에도 남긴다. `run.py`는
   `sent_status == "failed"`면 종료 코드를 1로 남겨, 실제 발송 실패가 더 이상
   GitHub Actions에서 초록불로 끝나지 않는다.
3. **`ask_picks`/`draft`/`check` 호출에 예외 처리 없음** — 셋 다 `try/except`로
   감싸 실패·거부(`.parsed is None`) 시 "이 배치/이 기사/이 검수는 실패"로
   취급하고 나머지는 계속 진행하도록 바꿨다. `report`가 `Send`로 병렬 fan-out
   되는 구조라, 이전에는 기사 한 건의 API 오류가 전체 실행을 멈출 수 있었다.
4. **`check()`가 `why` 문장은 검수 안 함** — `SYS_CHECK`와 `check()`의 입력에
   `[왜 중요한지]`를 추가했다. 다만 `why`는 해석·의견일 수 있어 "중요하다고
   판단한 것" 자체는 문제 삼지 않고, 그 안에 원문에 없는 새 사실 주장이 있을
   때만 불합격 사유로 삼도록 프롬프트에 명시했다(순수 검수 강화가 아니라
   "사실과 해석을 가르는" 원래 설계 의도를 유지한 채 범위만 넓힌 것).
5. **RSS가 403 등을 돌려줘도 "0건"으로만 보임** — `collect()`에
   `resp.raise_for_status()`를 추가해 4xx/5xx 응답을 "응답 없음" 목록에 잡는다.
   실제로 403을 돌려주는 엔드포인트로 재현해 `dead`에 잡히는 것을 확인했다.
6. **이메일 HTML에 이스케이프 없음** — `build_email_html()`의 모든 필드에
   `html.escape()`를 적용했다. `<script>alert(1)</script>` 같은 제목을 넣고
   출력에 `<script>` 태그가 그대로 남지 않는 것을 직접 확인했다(보안 뉴스레터
   특성상 이런 제목이 실제로 들어올 수 있다는 지적이 정확했다).
7. **`event` 중복 제거가 예선 배치 단위로만 동작** — 수집이 `BATCH`(40)를
   넘어 예선이 두 배치 이상으로 나뉘면, 라벨이 배치마다 따로 붙어 같은 사건도
   다른 문자열이 될 수 있었다. `relabel_events()`를 추가해 배치가 둘 이상일
   때만 생존자 전체를 한 화면에 다시 놓고 라벨을 통일한다. 수집 76건(2배치)으로
   강제 재현해 로그에 `event 재통일`이 찍히는 것과, 예외 없이 끝까지 도는 것을
   확인했다.
8. **REPORT 8번의 타임아웃 서술 오류** — "본문 추출도 20초 타임아웃"은 사실이
   아니었다(`trafilatura.fetch_url()`은 `requests`의 `timeout`을 안 씀). 8번
   항목 본문을 정정했다.

**고치지 않고 남겨둔 것**: 검수를 고정 평가셋(조작 요약 10건 + 정상 요약 10건,
3회 판정)으로 수치화하자는 제안은 맞는 방향이라고 보지만, 오늘은 위 8건을
우선 처리했다. 다음 개선 과제로 남긴다.
