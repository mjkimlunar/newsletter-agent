...
만큼만 열린다. 네트워크 호출(`requests.get(..., timeout=20)`, RSS·본문 추출·웹훅
전송 전부)에는 20초 타임아웃이 걸려 있어 한 소스가 응답을 안 해도 전체가 멈추지
않는다. 운영 레벨에서는 `.github/workflows/security.yml`에 `timeout-minutes: 20`
잡 타임아웃이 걸려 있어, 위 장치가 다 뚫려도 GitHub Actions가 강제 종료한다.
